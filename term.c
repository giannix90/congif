...
static info_backup backup;// used for back up of previous word when i use the scroll
...
static void
save_cursor(Term *term) //i save the cursor
{
...
reset(Term *term)
{
    ...
    term->row = term->col = 0; //set to 0 
    term->top = 0; //top of the terminal
    term->bot = term->rows - 1; //bottom
    term->mode = def_mode;
    term->attr = def_attr;
    term->pair = def_pair;
    ...
    for (i = 0; i < term->rows; i++) {
        ...
        for (j = 0; j < term->cols; j++)
            term->addr[i][j] = (Cell) {EMPTY, def_attr, def_pair};  //set to default all cells of the terminal
    }
    save_cursor(term);
    save_misc(term);
}
...
{
    size_t size = sizeof(Term) + rows*sizeof(Cell *) + rows*cols*sizeof(Cell); //set the size for the terminal 
    Term *term = malloc(size); // allocate the size for the terminal
    if (!term)
        return NULL;
    term->rows = rows;
    term->cols = cols;
    term->addr = (Cell **) &term[1]; //address of the 1° block in the space reserved by malloc for rows*sizeof(Cell *)
    term->cells = (Cell *) &term->addr[rows]; //address of the 1° block in the space reserved by malloc for rows*cols*sizeof(Cell) =>each cell
    reset(term);
    return term;
}
...
static int //return 1 if the selected columns and row are between  bounds::: 0 otherwise
...
restore_term(Term* term,int x, int y,int mode)
{
    ...
    for(int i=0;i<14;i++){
            ...
            if(mode==1 || mode==3){
                term->addr[y][x+i]=backup.r1[i];
                backup.r1[i]=BLANK;
                backup.r1[i].code=' ';
            }

            if(mode==2 || mode==3){
                term->addr[y+1][x+i] =backup.r2[i];
                backup.r2[i]=BLANK;
                backup.r2[i].code=' ';
            }
    }
...
void 
add_info(Term * term,char* code,char* frame,int x,int y,int mode)
{
    char* info="time:";
    char* frame_n="frame:";

    /*if the terminal is full*/
    if (!within_bounds(term, term->row, term->col))
        return;

    if(mode==1 || mode==3){
        //I put the info on the bottom on the rigth
        for(int i=0;i<9;i++){
        
            Cell cell=(Cell){i<5?info[i]:code[i-5],term->attr,term->pair};
            cell.pair = cell.pair & 0xBB;//yellow background
            cell.pair = cell.pair >> 4;
            term->addr[y][x+i] = cell;
    ...
    if(mode==2 || mode==3){
        for(int i=0;i<6+strlen(frame);i++){
            
            Cell cell=(Cell){i<6?frame_n[i]:frame[i-6],term->attr,term->pair};
            cell.pair = cell.pair & 0xBB;//yellow background
            cell.pair = cell.pair >> 4;
            term->addr[y+1][x+i] = cell;
        }
    }
}
