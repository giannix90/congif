...
typedef struct Term { //struct associated at the terminal
    int rows, cols;
    int row, col;
    int top, bot;
    uint16_t mode;
...
Term *new_term(int rows, int cols); //create a new terminal
void parse(Term *term, uint8_t byte);
void add_info(Term * term,char* code,char* frame,int x,int y,int mode);
void restore_term(Term* term,int x,int y,int mode);
void save_info(Term *term,int x,int y,int mode);
